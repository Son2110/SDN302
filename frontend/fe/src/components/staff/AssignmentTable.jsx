      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm text-gray-600">
          <thead className="bg-gray-50 text-gray-700 font-medium">
            <tr>
              <th className="px-5 py-4 border-b">Đơn (Booking)</th>
            ...
            {assignments.map((a) => (
              <tr key={a._id} className="hover:bg-gray-50 transition-colors">
                <td className="px-5 py-4">
                  ...
                  <div className="text-sm mt-1">{a.booking?.customer?.user?.full_name}</div>
                  <div className="text-xs text-gray-400 mt-0.5">
                    {formatDate(a.booking?.start_date)} - {formatDate(a.booking?.end_date)}
                  </div>
                </td>
                <td className="px-5 py-4">
                ...
                <td className="px-5 py-4">
                  <div className="font-medium text-gray-900">{a.driver?.user?.full_name || "N/A"}</div>
                  <div className="text-xs text-gray-500 mt-1">{a.driver?.user?.phone}</div>
                </td>
                <td className="px-5 py-4">
                  <span
                ...
                <td className="px-5 py-4">
                  <div className="text-xs text-gray-500">{formatDate(a.assigned_at, true)}</div>
                  <div className="text-xs text-blue-600 mt-1 w-20 truncate" title={a.assigned_by?.user?.full_name}>
