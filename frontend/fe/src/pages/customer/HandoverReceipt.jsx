...
const HandoverReceipt = () => {
    ...
    const handleConfirmReceipt = async () => {
        if (!deliveryHandover?._id) return;

        try {
            setConfirming(true);
            await confirmDeliveryReceipt(deliveryHandover._id);
            await loadData();
        } catch (err) {
        ...
        <div className="min-h-screen bg-gray-50 pt-32 pb-20">
            <div className="max-w-4xl mx-auto px-6 space-y-6">
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    <h1 className="text-2xl font-bold text-gray-900">Vehicle delivery report</h1>
                ...
                {!deliveryHandover ? (
                    <div className="bg-white rounded-2xl shadow-sm border border-red-200 p-6">
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <p className="text-red-700 font-semibold">Delivery report not available yet</p>
                                <p className="text-sm text-gray-600 mt-1">
                                    Staff has not created the delivery handover for this booking.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-lg font-bold text-gray-900">Delivery handover details</h2>
                            </div>
                        ...
                        {!deliveryHandover.confirmed_by_customer ? (
                            ...
                            >
                                {confirming ? "Confirming..." : "Confirm vehicle receipt"}
                            </button>
                        ) : (
                            <div className="flex items-center gap-2 text-blue-700 font-semibold">
                                <CheckCircle className="w-5 h-5" />
                                You have confirmed this delivery report.
                            </div>
                        )}
                    </div>
                )}
